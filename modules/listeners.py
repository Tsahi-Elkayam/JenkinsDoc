...
from .log import LOG
from .utils import get_jenkins_data, get_priority_snippets, get_settings, is_jenkins_file
...
class JenkinsDocStatusBar(sublime_plugin.EventListener):
    ...
    def on_close(self, view):
        """Drop throttle entry to keep dict bounded."""
        self._last_update.pop(view.id(), None)
...
class JenkinsCompletions(sublime_plugin.EventListener):
    ...
    def on_query_completions(self, view, prefix, locations):
        ...
        LOG.debug("on_query_completions: prefix=%r file=%s", prefix, view.file_name())

        try:
            if not is_jenkins_file(view, settings):
                LOG.debug("  not a Jenkins file - returning None")
                return None

            LOG.debug("  is Jenkins file - building completions")
            ...
            # 1. env.<var> context
            if jenkins_data and re.search(r"env\.\w*$", line_up_to_cursor):
                env_completions = self._get_env_completions(jenkins_data, include_prefix=False)
                if env_completions:
                    LOG.debug("  returning %d env completions", len(env_completions))
                    return (env_completions, sublime.INHIBIT_WORD_COMPLETIONS)

            # 2. inside post {} block -> post-condition completions
            if jenkins_data and self._is_inside_post_block(view, point):
                post_completions = self._get_post_completions(jenkins_data)
                if prefix:
                    pl = prefix.lower()
                    post_completions = [c for c in post_completions if c[0].split("\t")[0].lower().startswith(pl)]
                if post_completions:
                    LOG.debug("  returning %d post completions", len(post_completions))
                    return (post_completions, sublime.INHIBIT_WORD_COMPLETIONS)

            # 3. parameter context inside fn( ... )
            if jenkins_data and "(" in line_up_to_cursor and not line_up_to_cursor.rstrip().endswith(")"):
                param_completions = self._get_parameter_completions(jenkins_data, line_up_to_cursor)
                if param_completions:
                    LOG.debug("  returning %d parameter completions", len(param_completions))
                    return (param_completions, sublime.INHIBIT_WORD_COMPLETIONS)

            # Default: priority + instructions + sections + directives
            completions = []
            priority_commands = get_priority_snippets()
            added_commands = set()
            ...
            if jenkins_data and jenkins_data.get("instructions"):
                LOG.debug("  adding %d instructions from jenkins_data", len(jenkins_data["instructions"]))
            ...
            LOG.debug("  total completions: %d", len(completions))
            if LOG.isEnabledFor(10) and completions:  # 10 == logging.DEBUG
            ...
            LOG.debug("  no completions - returning None")
            return None

        except Exception:
            LOG.exception("error in on_query_completions")
            return (
                [
    ...
    def _get_instruction_completions(self, jenkins_data, settings, prefix="", added_commands=None):
        ...
        if not jenkins_data or not jenkins_data.get("instructions"):
            LOG.debug("no instructions data available")
            return completions
        ...
        LOG.debug("created %d instruction completions (prefix=%r)", len(completions), prefix)
        return completions
    ...
    def _get_section_completions(self, jenkins_data):
        """Return section completions, with inner-instruction hints when known."""
        completions = []
        if not jenkins_data:
            return completions
        for section in jenkins_data.get("sections", []):
    ...
    def _get_directive_completions(self, jenkins_data):
        """Return directive completions, with inner-instruction hints when known."""
        completions = []
        if not jenkins_data:
            return completions
        for directive in jenkins_data.get("directives", []):
    ...
    def _get_post_completions(self, jenkins_data):
        """Return post-condition completions for use inside ``post {}`` blocks."""
    ...
    def _get_parameter_completions(self, jenkins_data, line_up_to_cursor):
        """Return parameter completions when the cursor is inside ``fn(...)``."""
    ...
    def _is_inside_post_block(self, view, point):
        """Return True when ``point`` sits inside the body of a ``post {...}`` block."""
...
class JenkinsGoToDefinitionCommand(sublime_plugin.EventListener):
    ...
    def _get_groovy_files(self, folder):
        """Get cached list of Groovy / Jenkinsfile sources or rebuild cache."""
        import time
        ...
        for root, dirs, files in os.walk(folder):
            ...
            for file in files:
                if file.endswith(".groovy") or file == "Jenkinsfile" or file.startswith("Jenkinsfile."):
        ...
        if "." in extended_word:
            ...
            if len(parts) == 2:
                ...
                if settings and settings.get("show_goto_popup", True):
                    safe_label = html.escape(extended_word)
                    safe_file = html.escape(file_name)
                    safe_fn = html.escape(function_name)
                    link_html = '<a href="goto:{0}:{1}">Go to definition of {2}</a>'.format(
                        safe_file, safe_fn, safe_label
                    )
                    view.show_popup(
                        link_html,
                        flags=sublime.HIDE_ON_MOUSE_MOVE_AWAY,
    ...
    def _goto_definition(self, view, file_or_function, function_name=None):
        ...
        File-discovery paths (os.walk + file reads) are dispatched to
        Sublime's worker thread so large repos don't freeze the UI.
        """
        window = view.window()
        if not window:
            return
