...
class TestCommandsRegistry(unittest.TestCase):
    ...
    def test_command_classes_exist(self):
        ...
        for cmd in self.commands:
            ...
            self.assertIn(command_name, expected_commands,
                         f"Command '{command_name}' is not in expected commands list")

    def test_command_names_follow_convention(self):
        """Test that command names follow snake_case convention"""
        for cmd in self.commands:
            command_name = cmd['command']
            self.assertTrue(command_name.islower() or '_' in command_name,
                           f"Command '{command_name}' doesn't follow snake_case")
